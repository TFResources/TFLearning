...
def read_data_sets(param, one_hot=False, dtype=dtypes.float32, reshape=True,validation_size=5000):
    ...
    local_file = os.path.join("../tmp/", TRAIN_IMAGES)
    with open(local_file, 'rb') as f:
        train_images = extract_images(f)

    local_file = os.path.join("../tmp/", TRAIN_LABELS)
    with open(local_file, 'rb') as f:
        train_labels = extract_labels(f, one_hot=one_hot)

    local_file = os.path.join("../tmp/", TEST_IMAGES)
    with open(local_file, 'rb') as f:
        test_images = extract_images(f)

    local_file = os.path.join("../tmp/", TEST_LABELS)
    with open(local_file, 'rb') as f:
